...
/**
 * @custom:name XRPPaymentNonexistence
 * @custom:id 0x09
 ...
 *
 *
 * This type of attestation can be used to e.g. provide grounds to liquidate funds locked by a
 * smart contract on Flare when a payment is missed.
 *
 ...
 *   - Transaction status is `SUCCESS` and the amount received by the specified destination address is greater than
 *     the specified `value`.
...
interface IXRPPaymentNonexistence {
     ...
     * @param responseBody Data defining the response. The verification rules for the construction of
     * the response body and the type are defined per specific `attestationType`.
     */
    struct Response {
        bytes32 attestationType;
        bytes32 sourceId;
     ...
     * @custom:below If both `firstMemoDataHash` and `destinationTag` are zero, they are not considered in
     * the search and are ignored, effectively looking for any transaction with at least the specified
     * amount sent to the specified address.
     */
    struct RequestBody {
