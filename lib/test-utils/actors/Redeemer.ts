...
export class Redeemer extends AssetContextClient {
    ...
    async requestRedemptionAnyAmount(amount: BNish, executorAddress?: string, executorFeeNatWei?: BNish): Promise<[requests: EventArgs<RedemptionRequested>[], remainingLots: BN, dustChanges: EventArgs<DustChanged>[]]> {
        const executorFee = executorAddress ? toBN(requireNotNull(executorFeeNatWei, "executor fee required if executor used")) : undefined;
        const res = await this.assetManager.redeemAmount(amount, this.underlyingAddress, executorAddress ?? ZERO_ADDRESS,
            { from: this.address, value: executorFee });
        const redemptionRequests = filterEvents(res, 'RedemptionRequested').map(e => e.args);
        const redemptionIncomplete = optionalEventArgs(res, 'RedemptionAmountIncomplete');
        const dustChangedEvents = filterEvents(res, 'DustChanged').map(e => e.args);
        const remainingAmount = redemptionIncomplete?.remainingAmountUBA ?? BN_ZERO;
        return [redemptionRequests, remainingAmount, dustChangedEvents];
    }
