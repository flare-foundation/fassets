...
contract(`Whitelist.sol; ${getTestFile(__filename)}; Whitelist basic tests`, accounts => {
    ...
    describe("whitelist functions", () => {
        ...
        it('should not add addresses if not governance or manager', async function () {
            const res = whitelist.addAddressesToWhitelist(whitelistedAddresses);
            await expectRevert(res, "only governance or manager");
        });

        it('should not add address if not governance or manager', async function () {
            const res = whitelist.addAddressToWhitelist(accounts[5]);
            await expectRevert(res, "only governance or manager");
        });

        it('should not add address 0', async function () {
            const res = whitelist.addAddressToWhitelist(ZERO_ADDRESS, {from: governance});
            await expectRevert(res, "address zero");
        });

        it('should add addresses to the whitelist', async function () {
            const res = await whitelist.addAddressesToWhitelist(whitelistedAddresses, {from: governance});
        ...
        it('should not add addresses to the whitelist twice', async function () {
            const res = await whitelist.addAddressesToWhitelist(whitelistedAddresses, { from: governance });
            expectEvent(res, "Whitelisted");
            const res2 = await whitelist.addAddressesToWhitelist(whitelistedAddresses, { from: governance });
            expectEvent.notEmitted(res2, "Whitelisted");
            const res3 = await whitelist.addAddressToWhitelist(whitelistedAddresses[0], { from: governance });
            expectEvent.notEmitted(res3, "Whitelisted");
        });

        it('should revoke addresses from the whitelist', async function () {
            const res_1 = await whitelist.addAddressToWhitelist(whitelistedAddresses[0], {from: governance});
            const res_2 = await whitelist.addAddressToWhitelist(whitelistedAddresses[1], {from: governance});
            const isWhitelisted0 = await whitelist.isWhitelisted(whitelistedAddresses[0]);
            const isWhitelisted1 = await whitelist.isWhitelisted(whitelistedAddresses[1]);
        ...
        it('should not revoke addresses from the whitelist twice', async function () {
            const res = await whitelist.addAddressesToWhitelist(whitelistedAddresses, { from: governance });
            expectEvent(res, "Whitelisted");
            const res2w = await whitelist.revokeAddress(whitelistedAddresses[0], { from: governance });
            expectEvent(res2w, "WhitelistingRevoked");
            const res3w = await whitelist.revokeAddress(whitelistedAddresses[0], { from: governance });
            expectEvent.notEmitted(res3w, "WhitelistingRevoked");
            const res4w = await whitelist.revokeAddress(accounts[5], { from: governance });
