...
contract(`AssetManager.sol; ${getTestFile(__filename)}; Asset manager integration tests - core vault`, accounts => {
    ...
    describe("transfer to core vault", () => {
    ...
    describe("return from core vault", () => {
    ...
    describe("direct redemption from core vault", () => {
    ...
    describe("core vault client checks and validations", () => {
    ...
    describe("core vault transfer operation after failure and default", () => {
    ...
    describe("core vault minimum amount left", () => {
    ...
    describe("core vault donation validations", () => {
        const donorAddress = "DonorUnderlying";

        beforeEach(async () => {
            mockChain.mint(donorAddress, 10e6);
        });

        it("should revert if sender is not the proof owner", async () => {
            const wallet = new MockChainWallet(mockChain);
            const coreVaultDonationTag = Number(context.initSettings.coreVaultDonationTag);
            const txHash = await wallet.addTransaction(donorAddress, coreVaultUnderlyingAddress, 1e6, null, { destinationTag: coreVaultDonationTag });
            const proof = await context.attestationProvider.proveXRPPayment(txHash, minterAddress1);
            await expectRevert.custom(context.assetManager.confirmCoreVaultDonation(proof, { from: minterAddress2 }), "OnlyProofOwner", []);
        });

        it("should revert if receiving address is wrong", async () => {
            const wallet = new MockChainWallet(mockChain);
            const coreVaultDonationTag = Number(context.initSettings.coreVaultDonationTag);
            const txHash = await wallet.addTransaction(donorAddress, underlyingAgent1, 1e6, null, { destinationTag: coreVaultDonationTag });
            const proof = await context.attestationProvider.proveXRPPayment(txHash, null);
            await expectRevert.custom(context.assetManager.confirmCoreVaultDonation(proof), "InvalidReceivingAddress", []);
        });

        it("should revert if destination tag is wrong", async () => {
            const wallet = new MockChainWallet(mockChain);
            const wrongTag = Number(context.initSettings.coreVaultDonationTag) + 1;
            const txHash = await wallet.addTransaction(donorAddress, coreVaultUnderlyingAddress, 1e6, null, { destinationTag: wrongTag });
            const proof = await context.attestationProvider.proveXRPPayment(txHash, null);
